...
class Get_w_rid_And_wts:
    def __init__(self, session):
        self.session = session

    def get_wbiImgKey_and_wbiSubKey(self):
        """
    ...
    def get_w_rid_And_wts(self, e: dict):
        """
        get w_rid and wts
        :param e: requests payload, each key must be a string and each value must be a string or None
        :return: w_rid, wts
        """
        wbiImgKey, wbiSubKey = self.get_wbiImgKey_and_wbiSubKey()
        t = wbiImgKey + wbiSubKey
        r = list()
        MIXIN_KEY_ENC_TAB = [46, 47, 18, 2, 53, 8, 23, 32, 15, 50, 10, 31, 58, 3, 45, 35, 27, 43, 5, 49, 33, 9, 42, 19,
        ...
        for key in c:
            ...
            if value is not None and type(value) == str:
                ...
                l.append(self.encodeURIComponent(key) + "=" + self.encodeURIComponent(value))
        v = "&".join(l)
