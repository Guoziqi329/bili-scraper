...
class GetArticle:
    ...
    def get_article_html(self, article_id: str):
        """
        get article html
    ...
    def get_img(self, article_id: str, url_list: list, path: Path) -> list:
        """
        get img.
    ...
    def add_text(self, doc, item: dict, align: int = 0) -> None:
        """
        ...
        for node in item:
            if node['type'] == 'TEXT_NODE_TYPE_WORD':
                ...
                if 'jump_url' in node['rich'].keys():
                    jump_url = node['rich']['jump_url']
                    if jump_url.startswith(('http://', 'https://')):
                        url = jump_url
                    else:
                        url = 'https://' + jump_url.lstrip('/')
                    self.add_hyperlink(paragraph, text, url)
                else:
    ...
    def add_auto_numbered_data(self, doc, items: list) -> None:
        """
    ...
    def add_link_card(self, doc, item: dict) -> None:
        """
        add link card.
        :param doc: docx document.
        ...
        elif 'common' in item['link_card']['card']:
            title_and_url = item['link_card']['card']['common']
        else:
            logging.error("Can not add link card")
            return

        self.add_hyperlink(paragraph, title_and_url['title'], title_and_url['jump_url'])
        doc.add_paragraph('\n')


    def get_article(self, article_id: str, doc_storage_location: str = None, document_name: str = 'Document.doc',
        ...
        html = self.get_article_html(article_id)
        ...
        title = self.get_module(INITIAL_STATE, 'title')
        author = self.get_module(INITIAL_STATE, 'author')
        module_top = self.get_module(INITIAL_STATE, 'module_top')
        module_content = self.get_module(INITIAL_STATE, 'module_content')
        ...
        self.add_image(doc, self.get_img(article_id, top_pic_urls, img_path))
        ...
        for item in module_content:
            if item['para_type'] == 1:
                self.add_text(doc, item['text']['nodes'], item['align'])
            elif item['para_type'] == 2:
                pic_urls = list()
                pics = item['pic']['pics']
                for pic in pics:
                    pic_urls.append(pic['url'])
                self.add_image(doc, self.get_img(article_id, pic_urls, img_path))
